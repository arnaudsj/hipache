(function () {
    ...
    var Config = function (source) {
        ...
        if (options.driver.match(/\$\w+/g) !== null) {
            var matches = options.driver.match(/\$\w+/g);
            for(var i=0; i < matches.length; i++){
              var envVar = matches[i];
              options.driver = options.driver.replace(envVar, process.env[envVar.slice(1)]);
            }
        }
    ...
    Config.prototype.prepareHttps = function (https) {
        ...
        https.bind = https.bind.map(function (item) {
            ...
            // If we share the same key as global, have the passphrase as well, unless overriden
    ...
    Config.prototype.prepareUser = function (user) {
        ...
        if (process.getuid() === 0) {
            ...
            // Otherwise, if he didn't explicitely required root, scream and quit
